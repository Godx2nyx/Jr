...
do
    ...
    for attempt = 1, 4 do
        ...
        warn(("[libercy] UI host attempt %d/4 failed: %s"):format(attempt, tostring(lastErr)))
        task.wait(attempt * 1.5)
    end
    if not Rayfield then
        warn("[libercy] Could not load the UI library. The menu host is down, not your executor.")
        warn("[libercy] Last error: " .. tostring(lastErr))
        libercySplash.fail("Menu host is down - try again in a minute")
        return
    end
end
libercyLoadT.lib = os.clock()
libercySplash.step("Waiting for the game", 0.3)
...
if not net then
    ...
    Rayfield:Notify({ title = "libercyHub", content = "Networking not found - are you in Steal An Egg?",
...
if not (EggState and GuardAreas) then
    ...
    Rayfield:Notify({ title = "libercyHub", content = "Game not fully loaded - rejoin and try again",
        icon = "rbxassetid://95108798243406", duration = 6 })
    return
end
...
local Window = Rayfield:CreateWindow({
    ...
    configuration = {
        autoSave = false,
        autoLoad = false,
        fileName = "libercyHub_StealAnEgg",
    },
})
...
pcall(function()
    ...
    for _ = 1, 20 do
        ...
        if ci and ci.Parent and ci.Parent.Name == "libercyHub" then
    ...
    if sg then
        sg.Enabled = false
        libercySplash.whenClosed(function() sg.Enabled = true end)
    end
end)
local TRACE = true
...
do
    ...
    if #missing > 0 then
        ...
        pcall(function()
            Rayfield:Notify({ title = "libercyHub", content = msg,
...
;(function()
    ...
    libercyTop.pageEnter = layout.PageEnter:Connect(function(page)
        if liteMode then return end
        pcall(function()
            local sc = page:FindFirstChild("libercyPageScale") or Instance.new("UIScale")
            sc.Name = "libercyPageScale"
            sc.Parent = page
            sc.Scale = 0.975
