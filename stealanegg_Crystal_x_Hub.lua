...
local Z= "CrystalxHub_FlightSpeed.txt"
local z= "CrystalxHub_EggSelectConfig.json"
...
local function Mk(...) h.performanceMode = true pcall(function(...)
        local e=r:FindFirstChild( "CrystalxHub_EggESP" )
        if e then
            e:Destroy()
        end
...
local zk= "Crystal_x_Hub_Icon.png"
...
local function jM(...)
    local e=Instance.new ( "ScreenGui" )e.Name = "CrystalxHub_LOADER_SCREEN" e.ResetOnSpawn = false e.DisplayOrder = 9999999 e.ZIndexBehavior =Enum.ZIndexBehavior.Sibling e.AutoLocalize = false pcall(function(...)
    ...
    local w=Instance.new ( "TextLabel" )w.Size =UDim2.new ( 1 , -28 , 0 , 24 )w.Position =UDim2.new ( 0 , 14 , 0 , 14 )w.BackgroundTransparency = 1 w.Text = "Crystal x Hub" w.TextColor3 =Color3.fromRGB ( 245 , 248 , 255 )w.TextSize = 18 w.Font =Enum.Font.GothamBold w.TextXAlignment =Enum.TextXAlignment.Left w.AutoLocalize = false w.Parent =r
    ...
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "Initializing Crystal x Hub..." t.TextColor3 =Color3.fromRGB ( 130 , 140 , 165 )t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                break
            end
...
local kM={}kM.Gui =Instance.new ( "ScreenGui" )kM.Gui.Name = "CrystalxHub_RESTORE_BAR" kM.Gui.ResetOnSpawn = false kM.Gui.DisplayOrder = 999999 kM.Gui.ZIndexBehavior =Enum.ZIndexBehavior.Sibling kM.Gui.AutoLocalize = false pcall(function(...)
    if syn and syn.protect_gui then
...
local function aM(...) h.alive = false pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )
        ...
        local y=r:FindFirstChild( "CrystalxHub_EggESP" )
        if y then
            y:Destroy()
        end
    end
    ...
    pcall(function(...)
        for r,y in ipairs(game.CoreGui :GetChildren())do
            if y.Name :find( "CrystalxHub_" )or y.Name :find( "DesyncSniperUI" )or y.Name :find( "WindUI" )then
                y:Destroy()
            end
        end
    end
...
local function oM(...)
    ...
    local function j(e,...)
        ...
        if not y then
            pcall(function(...)
                (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(e.Title or "Crystal x Hub" ),[ "Text" ]=tostring(e.Content or "" );
                [ "Duration" ]= 3 })
            end
            )
        end
    ...
    if r then
        pcall(function(...)
            ...
            if e then
                r.Notify =function(r,y,...)
                    ...
                    if not u then
                        pcall(function(...)
                            (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(y and y.Title or "Crystal x Hub" );
        ...
        local p=r:CreateWindow({[ "Title" ]= "Crystal x Hub" ;
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "Crystal x Hub (Fallback)" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
...
H( "[+] Initializing Crystal x Hub.64 (Steal an Egg Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( true )C4()
    if o.Character then
        z4(o.Character )
    end
...
H( "[+] Crystal x Hub.64 Ready! All-In-One Built-in Anti-AFK & Prometheus Ready!" )
