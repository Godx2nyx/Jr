...
do
    ...
    for _, holder in ipairs({ env.__libercy_BX, env.__libercy_TOP }) do
    ...
    env.__libercy_TOP = {}
    env.__libercy_GEN = (tonumber(env.__libercy_GEN) or 0) + 1
end
libercyTop = ((type(getgenv) == "function" and getgenv()) or _G).__libercy_TOP
libercyLoadT = { start = os.clock() }
libercyRealPrint = libercyRealPrint or print
print = function(...)
    local env = (type(getgenv) == "function" and getgenv()) or _G
    if env.libercyDebug then libercyRealPrint(...) end
end
libercySplash = { step = function() end, discord = function() end,
...
;(function()
    local ok = pcall(function()
        ...
        local old = parent:FindFirstChild("libercySplash")
        if old then old:Destroy() end
        local gui = mk("ScreenGui", {
            Name = "libercySplash", DisplayOrder = 999997, IgnoreGuiInset = true,
        ...
        local title = mk("TextLabel", {
            ...
            TextXAlignment = Enum.TextXAlignment.Left, Text = "libercyHub",
        }, card)
        ...
        local function accept(opened)
            ...
            if opened then
                ...
                tryDiscordApp()
                print("[libercy] Discord: " .. INVITE_URL)
        ...
        setShown(true, 0.4)
        libercySplash.step = step
        libercySplash.discord = discord
        libercySplash.whenClosed = function(fn)
            if closedDone then pcall(fn) else onClosed[#onClosed + 1] = fn end
        end
        libercySplash.fail = function(why)
            if finished then return end
            status.Text = tostring(why)
            status.TextColor3 = Color3.fromRGB(240, 110, 110)
            close(4)
        end
        libercySplash.done = function()
            if finished then return end
            step("Ready", 1)
    ...
    if not ok then warn("[libercy] loading screen unavailable") end
end)()
libercySplash.step("Loading interface", 0.08)
local WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"))()
libercyLoadT.lib = os.clock()

-- WindUI compatibility layer for the original libercyHub controls.
-- Keeps the hub logic intact while replacing the old Rayfield-style UI API.
local function windNotify(opts)
    opts = opts or {}
    return pcall(function()
        WindUI:Notify({
            Title = opts.title or opts.Title or "libercyHub",
            Content = opts.content or opts.Content or "",
            Icon = opts.icon or opts.Icon,
            Duration = opts.duration or opts.Duration or 4,
        })
    end)
end

local function dummyControl(defaultValue)
    local c = { value = defaultValue }
    function c:Set(v)
        self.value = v
        return true
    end
    function c:Refresh(v)
        self.value = v
        return true
    end
    return c
...
local function wrapElement(native, kind, initialValue, window, flag)
    local proxy = { value = initialValue, _native = native }
    function proxy:Set(v, silent)
        proxy.value = v
        if kind == "Dropdown" and type(native.Select) == "function" then
            return native:Select(v)
        elseif type(native.Set) == "function" then
            return native:Set(v)
        end
    end
    function proxy:Refresh(v)
        if type(native.Refresh) == "function" then
            return native:Refresh(v)
        end
        proxy.value = v
    end
    setmetatable(proxy, { __index = native })
    if flag then
        window.controls = window.controls or {}
        window.controls[flag] = proxy
    end
    return proxy
end

local function makeTabAdapter(tab, window)
    local A = {}
    A._tab = tab
    A._window = window

    function A:CreateSection(o)
        -- Sections are intentionally omitted; WindUI tabs already provide grouping.
        return dummyControl(nil)
    end

    function A:CreateText(o)
        -- Legacy labels/text are not needed in the WindUI conversion.
        return dummyControl(o and (o.text or o.Text))
    end

    function A:CreateLabel(o)
        return dummyControl(o and (o.text or o.Text or o.name or o.title))
    end

    function A:CreateParagraph(o)
        return dummyControl(o and (o.content or o.text or o.title or o.name))
    end

    function A:CreateButton(o)
        o = o or {}
        return tab:Button({
            Title = o.name or o.title or "Button",
            Desc = o.description or o.desc,
            Callback = o.callback or o.Callback or function() end,
        })
    end

    function A:CreateToggle(o)
        o = o or {}
        local flag = o.flag or o.Flag
        local value = o.value
        if value == nil then value = o.Default end
        if value == nil then value = false end
        local proxy
        local callback = o.callback or o.Callback or function() end
        local native = tab:Toggle({
            Title = o.name or o.title or "Toggle",
            Desc = o.description or o.desc,
            Value = value,
            Flag = flag,
            Callback = function(v)
                if proxy then proxy.value = v end
                callback(v)
            end,
        })
        proxy = wrapElement(native, "Toggle", value, window, flag)
        return proxy
    end

    function A:CreateDropdown(o)
        o = o or {}
        local flag = o.flag or o.Flag
        local values = o.options or o.Values or {}
        local value = o.value
        if value == nil then value = o.default or o.Default end
        local proxy
        local callback = o.callback or o.Callback or function() end
        local native = tab:Dropdown({
            Title = o.name or o.title or "Dropdown",
            Desc = o.description or o.desc,
            Values = values,
            Value = value,
            Flag = flag,
            Multi = o.multi or o.Multi or false,
            Callback = function(v)
                if proxy then proxy.value = v end
                callback(v)
            end,
        })
        proxy = wrapElement(native, "Dropdown", value, window, flag)
        return proxy
    end

    function A:CreateInput(o)
        o = o or {}
        local flag = o.flag or o.Flag
        local value = o.value
        if value == nil then value = o.Default end
        local proxy
        local callback = o.callback or o.Callback or function() end
        local native = tab:Input({
            Title = o.name or o.title or "Input",
            Desc = o.description or o.desc,
            Placeholder = o.placeholder or o.Placeholder or "",
            Value = value,
            Flag = flag,
            Callback = function(v)
                if proxy then proxy.value = v end
        ...
        proxy = wrapElement(native, "Input", value or "", window, flag)
        return proxy
    end

    setmetatable(A, { __index = tab })
    return A
end

local Window = WindUI:CreateWindow({
    Title = "libercy Hub | Steal an Egg [Premium]",
    Icon = "rbxassetid://95108798243406",
    Author = "Quality-Team",
    Folder = "libercyHub_StealAnEgg",
...
function Window:CreateTab(o)
    o = o or {}
    local tab = self:Tab({
        Title = o.name or o.title or o.Title or "Tab",
        Icon = o.icon or o.Icon,
        Locked = o.locked or o.Locked or false,
    })
    return makeTabAdapter(tab, self)
end

function Window:CreateTag(o)
    -- WindUI's tag presentation is optional; do not create legacy labels.
    return dummyControl(o and (o.title or o.Title))
end

function Window:ChangeTheme(theme)
    local name = type(theme) == "string" and theme or "Dark"
    return pcall(function() WindUI:SetTheme(name) end)
end
...
function Window:Unload()
    self.unloaded = true
    pcall(function()
        if self.Destroy then self:Destroy() elseif self.UI then self.UI:Destroy() end
    end)
end

function Window:Notify(opts)
    return windNotify(opts)
end

function Window:Toast(opts)
    return windNotify(opts)
end

Window.minimiseAction = { callback = function() Window:ToggleHide() end }

;((type(getgenv) == "function" and getgenv()) or _G).__libercy_WINDOW = Window
...
if not net then
    libercySplash.fail("Networking not found - are you in Steal An Egg?")
    windNotify({ title = "libercyHub", content = "Networking not found - are you in Steal An Egg?",
        icon = "rbxassetid://95108798243406", duration = 6 })
    return
end
libercySplash.step("Hooking networking", 0.38)
...
libercySplash.step("Loading egg data", 0.45)
...
if not (EggState and GuardAreas) then
    libercySplash.fail("Game not fully loaded - rejoin and try again")
    windNotify({ title = "libercyHub", content = "Game not fully loaded - rejoin and try again",
        icon = "rbxassetid://95108798243406", duration = 6 })
    return
end
libercySplash.step("Mapping guard areas", 0.55)
libercyLoadT.game = os.clock()
libercySplash.discord()
libercyLoadT.discord = os.clock()
libercySplash.step("Building menu", 0.65)
pcall(function()
    local env = (type(getgenv) == "function" and getgenv()) or _G
    if env.__libercy_WINDOW and not env.__libercy_WINDOW.unloaded then
        env.__libercy_WINDOW:Unload()
    end
end)
-- WindUI window was created above; keep it as the single hub window.
-- WindUI owns the main ScreenGui; no Rayfield-specific collapsed-icon handling.
local TRACE = true
local TRACE_FILE = "libercyHub_trace.txt"
local traceBuf = {}
...
local function trace(msg)
    ...
    local debugOn = (type(getgenv) == "function" and getgenv().libercyDebug) or _G.libercyDebug
    if debugOn and (not liteMode or important) then
        ...
        if traceBuf.printSec ~= sec then
            ...
            if (traceBuf.printMuted or 0) > 0 then
                print(("[libercy] (%d lines held back last second - the file has them all)")
...
trace("=== libercyHub Steal An Egg loaded ===")
do
    local missing = {}
    ...
    if #missing > 0 then
        ...
        pcall(function()
            windNotify({ title = "libercyHub", content = msg,
...
do
    ...
    function libercyStealDone(name, pick)
    ...
    task.spawn(function()
        local __gen = (getgenv and getgenv().__libercy_GEN) or _G.__libercy_GEN
        local lastText, lastAt = nil, -100
        while true do
            if ((getgenv and getgenv().__libercy_GEN) or _G.__libercy_GEN) ~= __gen then return end
            if #queue > 0 then
                task.wait(0.35)
                ...
                if not (text == lastText and os.clock() - lastAt < 4) then
                    ...
                    local ok, err = pcall(function()
                        if type(Window.Toast) == "function" then
                            ...
                            Window:Notify({ title = "libercyHub", content = text,
...
libercyTop.hideKey = UserInputService.InputBegan:Connect(function(input, gpe)
...
local calibMin, calibMax = 200, 800
...
local BX = {
    ...
    reassertCount = 0,
    REASSERT_GAP = 2.5,
    SWAP_ATTR = "libercyStealHum",
}
((type(getgenv) == "function" and getgenv()) or _G).__libercy_BX = BX
...
function BX.armKickLog()
    if _G.__libercy_KICKLOG then return end
    _G.__libercy_KICKLOG = true
                ...
                for _, d in ipairs(cg:GetDescendants()) do
                    if d:IsA("TextLabel") and type(d.Text) == "string"
                       ...
                       and not seen[d.Text] then
                        seen[d.Text] = true
                        print("[libercy-DIALOG] (UNRELIABLE - may be a leftover from a previous session) "
    ...
    pcall(function()
        ...
        for _, r in ipairs(net:GetChildren()) do
            if r:IsA("RemoteEvent") then
                local nm = r.Name
                libercyTop[#libercyTop + 1] = r.OnClientEvent:Connect(function(...)
...
pcall(function()
    libercyTop.jump = UserInputService.JumpRequest:Connect(function()
    ...
    while stealing do
        ...
        if BX.riftOnly then
            ...
            if want and next(want) == nil then
                ...
                libercyNote("rift-covered", BX.riftAutoTrade
            ...
            if pick then
                ...
                elseif os.clock() - (BX.riftWaitAt or 0) > 8 then
                    ...
                    libercyNote("rift-wait", "Rift: waiting for " .. tostring(waitingFor), 120)
                end
                task.wait(1)
                continue
            end
        ...
        if selectedEggUid and (not target) and (not BX.followBest) then
            ...
            if BX.pickWasUser then
                ...
                libercyPickPrompt("Egg done - pick your next one")
            end
        elseif target then
            BX.selHoldSince = nil
            BX.selHoldLast = nil
        ...
        if not selectedEggUid and not BX.followBest then
            if not BX.saidPickOne then
                ...
                libercyPickPrompt("Pick an egg to steal")
            end
            task.wait(0.5)
            continue
        end
...
libercyTop.charAdded = LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1)
    BX.acState = nil
    BX.acStates = {}
    BX.acScanFails = 0
...
task.spawn(function()
    ...
    for _ = 1, 40 do
        ...
        for _, root in ipairs({ (gethui and gethui()) or nil, game:GetService("CoreGui") }) do
            ...
            if ok and d and d:IsA("ImageLabel") and d.Parent and d.Parent.Name == "libercyHub" then
                icon = d
                break
            end
        end
        if icon then
            icon.Image = LOGO
            libercyTop.logoGuard = icon:GetPropertyChangedSignal("Image"):Connect(function()
...
HomeTab:CreateText({
    ...
    text = "V3.1\n"
        ...
        .. "- libercyHub logo on notifications and the hide pill\n"
        .. "\nV3\n"
...
libercySplash.step("Building Main", 0.7)
...
BX.espOn = false
BX.espPool = {}
K.ESP_NAME = "libercyESP"
K.ESP_REFRESH = 0.4
K.ESP_MAX = 250
;(function()
    local frames = 0
    libercyTop.liteFrames = RunService.Heartbeat:Connect(function() frames = frames + 1 end)
    ...
    task.spawn(function()
        ...
        pcall(function() libercyTop.liteFrames:Disconnect() end)
    end)
end)()
...
K.ESP_FADE_BAND = 250
K.ESP_VIS_HZ = 20
function libercyEspScale(dist)
    return math.clamp(1.25 - (tonumber(dist) or 0) / 800, 0.6, 1.25)
end
libercyTop.espVis = RunService.RenderStepped:Connect(function()
...
;(function()
    ...
    local scaleConn = RunService.RenderStepped:Connect(function()
        ...
        for _, c in pairs(cards) do
            if c.anchor and c.anchor.Parent then
                local s = libercyEspScale((c.anchor.Position - eye).Magnitude)
                if math.abs((c.lastScale or -1) - s) > 0.01 or c.lastH ~= c.baseH then
                    c.lastScale, c.lastH = s, c.baseH
                    c.scale.Scale = s
    ...
    task.spawn(function()
        local __gen = (getgenv and getgenv().__libercy_GEN) or _G.__libercy_GEN
        while true do
            if ((getgenv and getgenv().__libercy_GEN) or _G.__libercy_GEN) ~= __gen then
...
;(function()
    ...
    local function makeTag(p)
        local th = libercyThemeCurrent or {}
    ...
    local lastFadeAt = 0
    libercyTop.mateFade = RunService.RenderStepped:Connect(function()
        local now = os.clock()
    ...
    libercyTop.mateLeave = Players.PlayerRemoving:Connect(dropTag)
    task.spawn(function()
        local __gen = (getgenv and getgenv().__libercy_GEN) or _G.__libercy_GEN
        local lastBeacon = 0
        while true do
...
libercySplash.step("Building Farm", 0.78)
...
libercySplash.step("Building Event", 0.84)
...
X.rift.pick = EventTab:CreateDropdown({
    ...
    callback = function(v)
        ...
        if BX.riftPet then
            ...
            libercyNote("rift-pick", "Rift pet: " .. X.petName(id), 3)
        end
    end,
})
EventTab:CreateButton({
    ...
    callback = function()
        ...
        libercyNote("rift-refresh", (#out > 0) and ("Rift: " .. table.concat(X.petNames(out), ", ")) or "Rift: none out", 2)
    end,
})
EventTab:CreateToggle({
    ...
    callback = function(v)
        ...
        libercyNote("rift-auto", "Rift auto " .. (BX.riftOnly and "ON" or "OFF"), 2)
        local el = X.el and X.el.autoSteal
        if el then
...
EventTab:CreateToggle({
    ...
    callback = function(v)
        ...
        libercyNote("rift-trade-toggle", "Auto trade-in " .. (BX.riftAutoTrade and "ON" or "OFF"), 2)
    end,
})
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
               ...
               and os.clock() - (X.riftTradeAt or 0) > 5 then
                ...
                task.spawn(function()
                    ...
                    if r == "traded" then
                        ...
                        libercyNote("rift-traded", "Rift: traded in - Rift Egg added to your eggs", 5)
                    elseif r == "revealed" then
                        trace("rift: finished a pending reveal")
                    elseif r then
                        trace("rift: trade-in " .. tostring(r))
                        libercyNote("rift-trade-fail", "Rift trade-in " .. tostring(r), 60)
                    end
                end)
            end
            local reqs = st.Requirements or {}
            ...
            if ready and not lastReady then libercyNote("rift-ready", "Rift: all pets ready", 120) end
            lastReady = ready
            if lastBanner and st.BannerId ~= lastBanner then
                libercyNote("rift-banner", "Rift: new banner " .. tostring(st.BannerDisplayName or st.BannerId), 30)
            end
            ...
            if pc and pt and pc >= pt - 5 and lastPity and pc > lastPity then
                libercyNote("rift-pity", ("Rift: pity %d/%d"):format(pc, pt), 60)
            end
            lastPity = pc
        end)
    end
...
pcall(function()
    ...
    if ev then
        libercyTop.bannerRotated = ev.OnClientEvent:Connect(function()
            X.riftSnapAt = 0
            pcall(X.riftPaint)
        end)
    end
end)
libercySplash.step("Building Misc", 0.9)
...
do
    local function pal(bgTop, bgBottom, tab, element, field, accent, accentLight)
        ...
        libercyThemeRaw = libercyThemeRaw or {}
        libercyThemeRaw[t] = { bgTop = bgTop, bgBottom = bgBottom, element = element,
                             accent = accent, accentLight = accentLight }
        return t
    end
    function libercyApplyThemeExtras(pick)
        libercyThemeCurrent = (pick and libercyThemeRaw and libercyThemeRaw[pick]) or {
        ...
        if type(libercyOnTheme) == "function" then pcall(libercyOnTheme, libercyThemeCurrent) end
    end
    local THEMES = {
        ["Default"] = nil,
        ["Midnight"] = pal(
    ...
    task.defer(function()
        pcall(function()
            ...
            libercyApplyThemeExtras(THEMES.Mono)
            BX.theme = "Mono"
    ...
    local function applyBackground(id, fade)
        ...
        local ok, why = pcall(function()
            ...
            for _, g in ipairs(parent:GetChildren()) do
                if g:IsA("ScreenGui") and g.Name ~= "libercySplash" then
...
;(function()
    ...
    local STROKE_T = 0.45
    local POS_FILE = "libercyHub_stats_pos.json"
    local ICON_DIR = "libercyHub/icons"
    ...
    task.spawn(function()
        ...
        pcall(function()
            if not isfolder("libercyHub") then makefolder("libercyHub") end
    ...
    takeTheme(libercyThemeCurrent)
    function libercyOnTheme(raw)
        takeTheme(raw)
    ...
    local function build()
        local parent = (gethui and gethui()) or game:GetService("CoreGui")
        local old = parent:FindFirstChild("libercyStats")
        if old then old:Destroy() end
        gui = mk("ScreenGui", {
            Name = "libercyStats", DisplayOrder = 999990, ResetOnSpawn = false,
...
task.spawn(function()
    ...
    while true do
        task.wait(1)
        local env = (type(getgenv) == "function" and getgenv()) or _G
        if env.__libercy_GEN ~= __gen then return end
        local closed = false
        ...
        if closed then
            ...
            pcall(stopProtect)
            pcall(disableNoclip)
            for _, holder in ipairs({ BX, libercyTop }) do
                for _, v in pairs(holder) do
                    if typeof(v) == "RBXScriptConnection" then
                        pcall(function() v:Disconnect() end)
                    end
...
libercySplash.step("Loading profiles", 0.95)
...
-- WindUI handles tab transitions natively.
pcall(function()
    Window.minimiseAction.callback = function() Window:ToggleHide() end
    trace("ui: WindUI hide action ready")
end)
pcall(function()
    local L, now = libercyLoadT, os.clock()
...
if false then task.spawn(function()
    ...
    local gui = new("ScreenGui", {
        Name = "libercyWelcome", DisplayOrder = 999999, IgnoreGuiInset = true,
        ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
    }, parent)
